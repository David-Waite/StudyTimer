        ...
        if (userDoc.exists()) {
          ...
          if (userData.vehiclesOwned.length != vehiclesData.List.length) {
            for (let vehicle of allVehicles) {
              // Check if the vehicle is already owned by the user
              let isVehicleOwned = userData.vehiclesOwned.some(
                (userVehicle) => userVehicle.name === vehicle.name
              )

              // If the vehicle is not owned by the user, add it to the user's vehicles with status 'buy'
              if (!isVehicleOwned) {
                userData.vehiclesOwned.push({
                  name: vehicle.name,
                  price: vehicle.price,
                  status: 'buy'
                })
