  ...
  <div class="navOuterContainer">
    <div class="navContainer" v-if="useRoute().path != '/login' && useRoute().path != '/signup'">
      <div>
        ...
        <RouterLink to="/shop" @click="openHelp && toggleOpen"><BIconShop /></RouterLink>
      </div>
      <div class="rightNav">
        <RouterLink v-if="$route.name === `settings`" to="/"><BIconGearFill /></RouterLink>

        <RouterLink @click="openHelp && toggleOpen v-if="$route.name === `home` || $route.name === 'shop'" to="/settings"
